...
def fill_X(auth_index):
    ...
    return gmm.predict_proba(features[k].reshape(1, -1))
...
gmm.fit(features[:25])
print(gmm.score(features[25:30]))
# pool
local_pool = multiprocessing.Pool(30)
X = local_pool.map(fill_X, range(len(four_surveys_taken_auth_ids)))
local_pool.close()
local_pool.join()
